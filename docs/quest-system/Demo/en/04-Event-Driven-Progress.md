...
Both exist on `UDemoQuestManagerComponent` and route through the server automatically, just like `NotifyQuestEvent`.
...
| `OnQuestTimerWarning(Quest, TimeRemaining)` | A timed quest crosses its own warning threshold (`UDemoQuestData::TimerWarningThreshold`). |
| `OnObjectiveTimerWarning(Quest, Objective, TimeRemaining)` | A timed objective crosses its own warning threshold (`UDemoQuestObjectiveData::TimerWarningThreshold`) — independent of the quest-level one. |
