...
| `ObjectiveType` | `EDemoQuestObjectiveType` | Яка дія завершує цю ціль — див. нижче. |
...
UDemoQuestBlueprintLibrary::NotifyCustomQuestEvent(PlayerState, "Craft", "HealthPotion", 1); // зарахується
UDemoQuestBlueprintLibrary::NotifyCollectEvent(PlayerState, "HealthPotion", 1);              // проігнорується
```
...
| `Objectives` | `TArray<UDemoQuestObjectiveData*>` | Цілі, з яких складається квест. |
| `PrerequisiteQuests` | `TArray<UDemoQuestData*>` | Квести, які мають бути завершені раніше. Автоматично перевіряються на цикли — див. [09](09-Validation-And-Cheats.md). |
| `CustomData` | `TMap<FName, FString>` | Довільні метадані (ID NPC, що видає квест, тег сюжетної лінії, …). |
| `Rewards` | `FDemoQuestReward` | Див. нижче. |
...
| `bAutoComplete` | `bool` (за замовчуванням `false`) | `true`: завершується миттєво, щойно виконані всі обов'язкові цілі. `false`: гравець має активно здати квест (див. [05 — Компоненти сцени](05-World-Components.md), `UDemoQuestReceiverComponent`). |
...
| `QuestSharingMode` | `EDemoQuestSharingMode` | Personal / Shared / Individual — див. [01](01-Core-Concepts.md). |
...
`FDemoQuestReward` — це лише *дані*, нічого не витрачається автоматично, і плагін ніяк не читає та не інтерпретує ключі `RewardAmounts` сам. Реальну видачу реалізуйте в `GiveQuestRewards()`, маршрутизуючи кожен ключ туди, де він щось означає у вашій грі; див. [02 — Інтеграція без налаштування](02-Zero-Config-Integration.md).
...
- `bAutoComplete = false`: квест стає *готовим до здачі* (`IsQuestReadyToTurnIn()` поверне `true`), але лишається `Active`, доки щось не викличе `CompleteQuest()` — як правило, `UDemoQuestReceiverComponent` на NPC чи скрині. Добре підходить для квестів із наративною розв'язкою в конкретному місці.

### Таймерні квести

Установіть `TimeLimit` у додатне число секунд. Після старту `UDemoQuestManagerComponent` кожен тік відлічує `TimeRemaining` для цього квесту. Автоматично відбуваються дві речі:
...
Залишковий час і квесту, і будь-якої таймерної цілі видно наживо у вкладці Quests оверлея `QuestSystemDemoDebug` — див. [09 — Валідація та чит-команди](09-Validation-And-Cheats.md).
