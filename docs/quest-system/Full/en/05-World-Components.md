...
Everything a level designer places in the world is a small, composable `UActorComponent` (or a tiny ready-made `AActor` wrapping one). Attach what you need to any actor — there is no required base class. Every class on this page lives in the `QuestSystemWorld` module (a separate module from Core `QuestSystem`, since none of it is Core quest logic — see [10 — C++ Integration](10-CPP-Integration.md) for the module list); include `QuestSystemWorldTypes.h` for all of it at once, or individual headers.
...
| Member | Purpose |
|---|---|
| `AvailableQuests` (`TArray<UQuestData*>`) | Quests this actor can offer. |
...
| `OnQuestAcceptedDelegate` | Multicast delegate for external listeners (e.g. a quest marker — see `UQuestMarkerComponentBase` below). |
...
| Member | Purpose |
|---|---|
| `AcceptedQuests` (`TArray<UQuestData*>`) | Quests that can be turned in here. |
...
`IQuestMarkerVisualInterface` (`Interfaces/QuestMarkerVisualInterface.h`) is the one shared contract behind the last two bullets: a single `BlueprintNativeEvent`, `OnQuestMarkerStateChanged(NewState, RelevantQuest)`, implementable by whatever object actually visualizes the state — an Actor (for `UQuestBlueprintMarkerComponent`) or a `UUserWidget` (for `UQuestWidgetMarkerComponent`) alike.

There's no mode switch, so nothing stops you from adding **several marker components to the same actor** — e.g. a `UQuestStaticMeshMarkerComponent` cone and a `UQuestWidgetMarkerComponent` name-tag at once, each tracking the same quest state independently and drawing its own thing.

### Adding your own marker type

Subclass `UQuestMarkerComponentBase` and override two protected virtuals: `SetupVisualization(Owner)` (called once from `BeginPlay`, after Giver/Receiver/Target siblings are cached — find or create whatever visual component you need) and `ApplyVisualization(State, RelevantQuest)` (called whenever the state changes — update it). The three built-in subclasses' own source are reference implementations of this pattern; if you just want "no built-in visual, only `OnMarkerStateChanged`", a one-off Blueprint subclass of `UQuestMarkerComponentBase` needs neither override.

A native C++ subclass with a typed `UWidgetComponent`/`UUserWidget` property needs a `UMG` dependency — that's exactly what `UQuestWidgetMarkerComponent` above already is, living in the `QuestSystemWorld` module (which depends on `UMG`, unlike core `QuestSystem`). Reach for a subclass of your own only when that built-in component genuinely doesn't fit — e.g. a marker driving more than a single mesh or widget component.

## UQuestInteractorComponent — the player side
...
1. A `UQuestReceiverComponent` with a quest ready to turn in → turns it in.
2. A `UQuestGiverComponent` with an available quest → accepts it (Personal quests go to the interacting player; Shared/Individual quests start for every currently connected player — see [06](06-Multiplayer.md)).
3. A `UQuestTargetComponent` that answers to the Interact trigger → fires that component's quest event.
...
## UQuestTargetComponent — the one smart world target
...
| `Interact` | the player uses `UQuestInteractorComponent` (the E key) near it. |
...
A batteries-included placeable actor: a visible mesh, a `UQuestTargetComponent`, and a `UQuestStaticMeshMarkerComponent` (auto-provisioning its own icon, see above) — ready to drop into a level and configure in the Details panel. Reach for this when the target is a dedicated prop you are placing purely for the quest (a pickup, a breakable, a touch-point). When the target is something that already exists in your game (an enemy, an NPC), add `UQuestTargetComponent` (and, if you want a marker, any of the marker subclasses above) to that actor directly instead — the actor is only a convenience wrapper.

If your game already has its own combat or inventory systems, you can also skip world actors entirely and call `NotifyKillEvent` / `NotifyCollectEvent` (or `UQuestTargetComponent::Trigger`) directly from that code.

## AQuestBoard / AQuestChest

Two more batteries-included placeable actors, for a giver-only or receiver-only prop: a visible mesh plus a `UQuestGiverComponent` (`AQuestBoard`, a notice board) or `UQuestReceiverComponent` (`AQuestChest`, a turn-in point), plus a `UQuestStaticMeshMarkerComponent` — ready to drop into a level and configure in the Details panel. Content-free like the rest of `Actors/`: `PropMesh` ships with no default mesh, so assign one per-instance or in a Blueprint subclass; the marker auto-provisions its own icon if you don't assign one either (see [Quest markers](#quest-markers) above). Configure `AvailableQuests` / `AcceptedQuests` directly on the giver/receiver component.

## Composing your own NPC

None of the above requires a special base class — compose whichever components you need on any `AActor` or `ACharacter`. A quest-giving-and-receiving NPC is just a `Character` with `UQuestGiverComponent` + `UQuestReceiverComponent` + `UQuestStaticMeshMarkerComponent` added on top — there's no dedicated ready-made class for this in the plugin (unlike `AQuestBoard`/`AQuestChest` above), since an NPC needs a skeletal mesh and animations, which are inherently game-specific content the plugin doesn't ship. Build it as a Blueprint (or a thin native `ACharacter` subclass, if you prefer C++) in your own project.

Look at `DemoMap` in the editor to see `AQuestBoard`/`AQuestChest` and a demo NPC Blueprint configured with real quests.
