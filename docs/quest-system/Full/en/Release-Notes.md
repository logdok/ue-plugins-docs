...
Browser — `UQuestData` and `UQuestObjectiveData`. A designer can build and change quest content
without opening the project in an IDE. See [03 — Authoring Quests](03-Authoring-Quests.md).

**No setup to integrate.** `UQuestWorldSubsystem` attaches a quest manager to every player and
...
**Everything is available from Blueprint.** The full `UQuestBlueprintLibrary` covers the same
