...
`UQuestDependencyValidator` runs a depth-first search over `PrerequisiteQuests` to catch self-dependencies (`A → A`) and circular chains (`A → B → C → A`), both for a single quest and for your whole database:
...
- **Editor-time**, on every `PrerequisiteQuests` edit and on save (`UQuestData::PostEditChangeProperty` / `PreSave` / `IsDataValid` — integrates with Unreal's Data Validation framework).
- **On world start**, via `UQuestWorldSubsystem`, when `bValidateQuestsOnStartup` is enabled (development builds only — see [02](02-Zero-Config-Integration.md)).
- **On demand**, via the `QuestValidate` cheat command below.
...
`UQuestValidationLibrary::LoadAllQuests()` scans the whole Asset Registry for every `UQuestData` asset. `FindQuestByID(QuestID)` uses it internally and is the standard way to resolve a quest from a save file or a console command:
...
`UQuestCheatManagerExtension` registers automatically with every `CheatManager` (non-shipping builds). It lives in the optional **`QuestSystemDebug`** module (alongside the debug overlay), so the core `QuestSystem` module stays free of any dev-only code. Use the commands from the in-game console. Each command that touches player state forwards itself to the server automatically if run on a client.

| Command | Usage | Purpose |
|---|---|---|
| `QuestValidate` | `QuestValidate` | Full dependency validation + diagnostics dump. |
| `QuestShowDependencies` | `QuestShowDependencies <QuestID>` | Print a quest's prerequisite tree. |
| `QuestShowDependents` | `QuestShowDependents <QuestID>` | Print quests that depend on this one. |
| `QuestShowChain` | `QuestShowChain <QuestID>` | Print the full (recursive) prerequisite chain. |
| `QuestCanAddPrereq` | `QuestCanAddPrereq <QuestID> <NewPrereqID>` | Check whether adding a prerequisite would create a cycle. |
| `QuestStats` | `QuestStats` | Aggregate stats: quests with prerequisites, average chain length, etc. |
| `QuestStart` | `QuestStart <QuestID>` | Force-start a quest on the calling player. |
| `QuestComplete` | `QuestComplete <QuestID>` | Force-complete a quest. |
| `QuestFail` | `QuestFail <QuestID>` | Force-fail a quest. |
| `QuestListActive` | `QuestListActive` | List the calling player's active quests. |
| `QuestListAvailable` | `QuestListAvailable` | List quests the player could start right now. |
| `QuestListCompleted` | `QuestListCompleted` | List the player's completed quests. |
| `QuestResetAll` | `QuestResetAll` | Clear all quest progress for the player. |
| `QuestReset` | `QuestReset <QuestID>` | Clear progress for one quest. |
| `QuestStatus` | `QuestStatus <QuestID>` | Full status report: objectives, timer, rewards, settings. |
| `QuestListAll` | `QuestListAll` | List every quest in the project. |
| `QuestFind` | `QuestFind <PartialName>` | Search quests by partial name/ID. |
| `QuestOverlay` | `QuestOverlay` | Toggle the translucent quest debug overlay (docked top-right, game keeps running; lists every quest with live state and per-card controls). |

`QuestOverlay` is one command on the same extension; it opens the debug overlay from the **`QuestSystemDebug`** module. You can also open it from code/Blueprint with `UQuestDebugLibrary::ToggleQuestDebugOverlay(PlayerController)` — bind it to any key.
