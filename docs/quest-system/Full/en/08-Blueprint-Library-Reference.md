...
| `CanStartQuest(Player, Quest)` | Prerequisites met + not already active/completed/failed (a failed quest needs `QuestReset` first). |
...
| `GetQuestStateText(State)` | Localized-ready text for `EQuestState`. |
| `GetObjectiveStateText(State)` | Localized-ready text for `EQuestObjectiveState`. |
