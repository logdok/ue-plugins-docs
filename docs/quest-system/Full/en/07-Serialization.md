...
QuestSystem does not ship a save system — it gives you a self-contained snapshot struct designed to be embedded into whatever `USaveGame` your game already uses.
...
`FQuestSystemSerializedState` (`Core/QuestSerialization.h`) contains everything needed to restore this manager's state:

| Field | Contents |
|---|---|
| `ActiveQuests` | Each active quest's path, state, timer, and per-objective progress (`FActiveQuestSaveData` / `FObjectiveProgressSaveData`). |
| `CompletedQuestPaths` / `FailedQuestPaths` | Paths to completed/failed quest assets. |
| `FinishedQuests` | Final per-objective snapshots for completed/failed quests (`FActiveQuestSaveData`) — the real end-state (e.g. `2/3`, which optional objectives were done) for a journal/history UI. Restored into the manager's `FinishedQuestRecords`; read it back with `GetObjectiveProgress` / `GetFinishedQuestObjectives`. |
...
public:
    ...
    UPROPERTY()
    FQuestSystemSerializedState PartyQuestState; // only relevant if you also persist shared state
};
