...
You do not need a custom `PlayerState`, `GameState`, or `GameMode` class to use QuestSystem. This chapter explains what does the wiring for you, and how to customize it when you need to.

## UQuestWorldSubsystem

`UQuestWorldSubsystem` is a `UWorldSubsystem` — it exists automatically in every game world, no setup required. On the server (a standalone single-player session counts as a server), it:

- creates a `UQuestManagerComponent` on the **GameState** (the party manager);
- creates a `UQuestManagerComponent` on **every `PlayerState`** as it spawns — this covers login, late join, seamless travel, and PIE;
...
If a `PlayerState` or `GameState` **already has** a `UQuestManagerComponent` — because you placed one manually in a custom class or Blueprint — the subsystem leaves it alone. Manual and zero-config setups can coexist.
...
In most code you will use `UQuestBlueprintLibrary` instead (see [08](08-Blueprint-Library-Reference.md)) — it wraps the same lookups with clearer names (`GetQuestManager`, `GetPartyQuestManager`).

## UQuestSystemSettings

Project Settings → **Game → Quest System** (stored in `DefaultGame.ini` under `[/Script/QuestSystem.QuestSystemSettings]`):
...
| `QuestManagerClass` | `UQuestManagerComponent` | The class the subsystem instantiates. Point this at your own subclass — see below. |
...
5. Project Settings → **Game → Quest System** → set `Quest Manager Class` to `BP_MyQuestManagerComponent`.
