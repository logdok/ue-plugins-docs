...
Універсальна точка входу — `NotifyQuestEvent`, доступна на компоненті-менеджері та, зручніше, як статичні функції `UQuestBlueprintLibrary`, що приймають `APlayerState*` і самі маршрутизують до потрібного менеджера:
...
Канонічні EventID також доступні як константи C++, щоб не набирати рядкові літерали: `QuestEvents::Kill`, `QuestEvents::Collect`, `QuestEvents::Location`, `QuestEvents::Interact` (у `Core/QuestTypes.h`).
...
Обидві є на `UQuestManagerComponent` і, як і `NotifyQuestEvent`, автоматично маршрутизуються через сервер.
...
| `OnQuestTimerWarning(Quest, TimeRemaining)` | Таймерний квест перетнув власний поріг попередження (`UQuestData::TimerWarningThreshold`). |
| `OnObjectiveTimerWarning(Quest, Objective, TimeRemaining)` | Таймерна ціль перетнула власний поріг попередження (`UQuestObjectiveData::TimerWarningThreshold`) — незалежно від порога квесту. |
