...
Кожен метод `UQuestManagerComponent`, що змінює стан (`StartQuest`, `CompleteQuest`, `FailQuest`, `AbandonQuest`, `UpdateObjectiveProgress`, `SetObjectiveProgress`, `NotifyQuestEvent`, …), усередині себе перевіряє авторитет і при виклику з клієнта перекидається на сервер через reliable RPC. Думати про це не потрібно ніколи — викликайте публічний метод звідки завгодно, з клієнта чи сервера, і все спрацює правильно.
