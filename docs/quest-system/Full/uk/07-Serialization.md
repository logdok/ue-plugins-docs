...
QuestSystem не постачає систему збережень — натомість він дає вам самодостатню структуру-знімок, спроєктовану для вбудовування в той `USaveGame`, який уже використовується у вашій грі.

## Експорт

```cpp
FQuestSystemSerializedState State = QuestManager->ExportState();
```

`FQuestSystemSerializedState` (`Core/QuestSerialization.h`) містить усе необхідне для відновлення стану цього менеджера:

| Поле | Вміст |
|---|---|
| `ActiveQuests` | Шлях, стан, таймер і прогрес за кожною ціллю для кожного активного квесту (`FActiveQuestSaveData` / `FObjectiveProgressSaveData`). |
| `CompletedQuestPaths` / `FailedQuestPaths` | Шляхи до завершених/провалених ассетів квестів. |
| `FinishedQuests` | Фінальні знімки прогресу цілей для завершених/провалених квестів (`FActiveQuestSaveData`) — реальний кінцевий стан (наприклад `2/3`, які опційні цілі були виконані) для журналу/історії. Відновлюються в `FinishedQuestRecords` менеджера; читаються через `GetObjectiveProgress` / `GetFinishedQuestObjectives`. |
...
public:
    UPROPERTY()
    FQuestSystemSerializedState PersonalQuestState;

    UPROPERTY()
    FQuestSystemSerializedState PartyQuestState; // актуально, лише якщо ви також зберігаєте спільний стан
};
