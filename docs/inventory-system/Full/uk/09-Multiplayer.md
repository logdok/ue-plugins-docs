...
Те саме стосується `AISLootContainer::Open` і `AISItemPickup::TryCollect` — їх слід
викликати із серверної логіки взаємодії (наприклад, з компонента взаємодії гравця, у
якого з'єднання є), а не напряму з клієнта.
