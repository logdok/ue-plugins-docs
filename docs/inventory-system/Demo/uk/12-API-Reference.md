...
без копії, та інтерфейс `IISDemoInventoryInterface`. Жодної **можливості** плагіна вони
не додають — обидва мають блюпринтові відповідники (`GetAllEntries` і пошук
компонента через `UISDemoInventoryBlueprintLibrary`).

---

## `UISDemoInventoryBlueprintLibrary` — скорочення
...
| `TryUseItem(Slot, Instigator)` | використати; повертає `EISDemoItemUseResult` |
...
`ExportState()` → `FISDemoInventorySaveData` · `ImportState(State)` *(сервер)*
...
## `UISDemoItemDefinition` — тип предмета
...
## `UISDemoItemInstance` — конкретна копія
...
|---|---|---|
| `UISDemoFragment_Durability` | `GetCurrentDurability(Instance)` | поточна міцність копії |
...
| `UISDemoFragment_Consumable` | `GetRemainingCharges(Instance)` | скільки зарядів лишилося |
| `UISDemoFragment_Equippable` | `AcceptsSlot(Slot)` | чи згоден предмет жити в цьому слоті |
...
## `UISDemoInventoryQueryLibrary` — пошук за описом
...
Поля `FISDemoInventoryQuery` (порожній запит підходить усьому, поля комбінуються через І):
...
| Інтерфейс | Навіщо |
|---|---|
| `IISDemoCollectibleInterface` | зробити своїм актором те, що вміє підбирати `UISDemoInventoryComponent::TryCollect`. `AISDemoItemPickup` уже його реалізує; свій — лише якщо ваш «підбирний» об'єкт не є його нащадком. Метод один: `TryGiveContents(Collector)`, викликається вже на сервері |
| `IISDemoInventoryInterface` | сказати, який із кількох інвентарів на акторі головний — `GetInventoryComponent`, `GetEquipmentComponent`, `GetAllInventoryComponents`. Потрібен рідко: `GetInventoryFor` і сам знаходить єдиний. *(лише C++)* |

---

## Об'єкти світу (`InventorySystemDemoWorld`)
...
| `TryCollect(Collector)` | підібрати — **лише на сервері**; з клієнта викликайте `UISDemoInventoryComponent::TryCollect` |
...
## `UISDemoInventoryWorldSubsystem` — стан рівня світу

Усе, що спільне для гри, але **не** для процесу: підписки та таймери, які мають
зникати разом зі світом. Отримати — `UISDemoInventoryWorldSubsystem::Get(WorldContext)`.
...
## Відлагодження (`InventorySystemDemoDebug`)
...
| Тип | Опис |
|---|---|
| `FISDemoItemStatEntry` | одне значення в екземплярі: тег + число |
| `FISDemoSlotRestriction` | правило для діапазону слотів |
| `FISDemoInventoryEntry` | зайнятий слот: індекс + предмет |
| `FISDemoEquipmentEntry` | зайнятий слот спорядження: тег + предмет + актор |
| `FISDemoLootTableEntry` | запис таблиці луту: предмет, діапазон кількості, шанс і вага |
| `FISDemoLootDrop` | результат кидка: предмет + точна кількість |
| `FISDemoInventoryQuery` | опис того, що шукаємо в інвентарі |
| `FISDemoItemSaveData` / `FISDemoInventorySaveData` / `FISDemoEquipmentSaveData` | збереження |

## Налаштування проєкту

`UISDemoInventorySettings` — **Project Settings → Game → Inventory System**. Усе
