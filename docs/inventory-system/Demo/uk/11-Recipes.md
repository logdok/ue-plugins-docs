...
Те саме працює для `UISDemoEquipmentComponent` і для будь-якого фрагмента.
...
або зберігайте посилання явно, або реалізуйте `IISDemoInventoryInterface` і поверніть із
