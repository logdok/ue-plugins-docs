...
(`Plugins/InventorySystemDemo/Content/Demo/Items/`), тож можна відкрити редактор і
...
Сім предметів уже створені в `Plugins/InventorySystemDemo/Content/Demo/Items/` — разом
...
Кладемо в рівень актора `AISDemoItemPickup` і в полі `Item Definition` вказуємо
...
на землі. Саме тому рядок **`pickup mesh: yes / no`** у дебаг-панелі (`DemoInvOverlay`)
...
**Перевірити без жодного рядка коду:** відкрийте `DemoInvOverlay` (клавіша `I` або
...
**Той самий крок без коду:** у `DemoInvOverlay`, на рядку меча в CONTENTS — кнопка
...
**Без коду:** кнопка **Equip** на рядку щита в `DemoInvOverlay` — так само, як і для
меча.
...
**Без коду:** кнопка **Equip** на рядку шолома в `DemoInvOverlay`, як і для решти.
...
| Усе про `DemoInvOverlay` і консольні команди | [13 — Відлагодження](13-Debugging.md) |
