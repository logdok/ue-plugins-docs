...
`ES3CredentialSource` має рівно чотири значення (`Core/S3Enums.h`). Таблиця нижче — про те,
...
3. **`Anonymous` проти приватного бакета дає `Permission Denied`, а не `Authentication Error`.**
   ...
   `MakeDefaultCredentialsProvider` навмисно **не** додає `Anonymous` у кінець ланцюжка.

---

## Провайдер, який сам ходить по нові ключі

`FS3CredentialsProvider_Callback` (`Auth/S3CredentialsProviders.h`) — саме те, що перетворює
...
2. Зчитайте його **в пам'ять**, а не у файл: `S3 Download Bytes` / `US3Client::DownloadBytes`.
...
1. **Скукайте вміст окремим контейнером.** Через чанки (Primary Asset Label) або окремий
   ...
   а не в рантаймі.
2. **Зчитайте контейнер у файл** — `S3 Download File` / `US3Client::DownloadFile`, кудись під
...
2. Ключі бере **кожен процес окремо**: `US3Subsystem` — це `UGameInstanceSubsystem`, у хоста
...
`S3CompatibleStorageDebug` присутній і в зібраній Development-збірці:
...
Підпис **синхронний**, а `FS3CredentialsProvider_Callback` може відповідати асинхронно. Якщо
...
`IS3CredentialsStore` (`Auth/IS3CredentialsStore.h`) поверх них і поставте свою реалізацію в
`FS3CredentialsProvider_LocalUserStore`. Решта плагіна при цьому не змінюється.
...
`MakeCredentialsProviderForSource` — єдине місце, яке перетворює «звідки брати ключі» на
