...
> `Auth Error`. Це не помилка плагіна — це джерело, обране не для того процесу.
...
> справді не спрацює, але помилка виглядатиме як `Auth Error` без очевидної причини. Якщо
...
|---|---|---|
| `Environment variables` у збірці для гравців | `Auth Error` на кожній операції | Це серверне джерело. Клієнту — `Anonymous`, `Local user store` або підписані посилання |
