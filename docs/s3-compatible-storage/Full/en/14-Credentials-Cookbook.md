...
|---|---|---|---|
| Keys the backend just returned, one storage | **Set Runtime Credentials** (on the subsystem) | `US3Subsystem::SetRuntimeCredentials` | The default client only |
| The same, but there are multiple storages | **Set Static Credentials** (on a client) | `US3Client::SetStaticCredentials` | Whichever client it's called on — including a profile's client |
| Keys need to be fetched fresh every time they expire | — | `US3Client::SetCredentialsProvider` with `FS3CredentialsProvider_Callback` | That client |
| The user entered keys that need to survive a restart | **Save S3 Credentials** | `US3CredentialsLibrary::SaveUserCredentials` | Every client with `Local user store` and this profile name |
| There are no keys and never will be | `Credential Source = Anonymous` | `FS3CredentialsProvider_Anonymous` | The client |

And the reverse actions:

| Task | Blueprint | C++ |
|---|---|---|
| Forget keys set from code | **Clear Runtime Credentials** | `US3Subsystem::ClearRuntimeCredentials` |
| Forget the user's keys (one profile / all) | **Clear S3 Credentials** / **Clear All S3 Credentials** | `US3CredentialsLibrary::ClearUserCredentials` / `ClearAllUserCredentials` |
| Force a profile's client to re-read its source | **Forget Profile Client** | `US3Subsystem::ForgetProfileClient` |
| Resolve keys right now, without passing anything | — | `US3Client::RefreshCredentials` |
...
`ES3CredentialSource` has exactly four values (`Core/S3Enums.h`). The table below is about
...
3. **`Anonymous` against a private bucket gives `Permission Denied`, not
   ...
   a provider at all. This is exactly why `MakeDefaultCredentialsProvider` deliberately does
...
`FS3CredentialsProvider_Callback` (`Auth/S3CredentialsProviders.h`) is exactly what turns "my
...
2. Read it **into memory**, not into a file: `S3 Download Bytes` /
   `US3Client::DownloadBytes`. An intermediate file has no role here — the bytes go straight to
   a decoder.
...
2. Keys are fetched **by each process on its own**: `US3Subsystem` is a
...
`S3CompatibleStorageDebug` module ships in a packaged Development build too:
...
Signing is **synchronous**, while `FS3CredentialsProvider_Callback` may answer asynchronously.
...
| Action | Node | C++ |
|---|---|---|
| Sign out of one account | **Clear S3 Credentials** (`default`) | `US3CredentialsLibrary::ClearUserCredentials` |
| Wipe everything | **Clear All S3 Credentials** | `US3CredentialsLibrary::ClearAllUserCredentials` |
| Show where the file lives | **Get S3 Credentials File Path** | `US3CredentialsLibrary::GetCredentialsFilePath` |
...
`IS3CredentialsStore` (`Auth/IS3CredentialsStore.h`) on top of them and plug your own
implementation into `FS3CredentialsProvider_LocalUserStore`. The rest of the plugin doesn't
change.
...
| What | Header |
|---|---|
| `ES3CredentialSource` — the four sources | `Core/S3Enums.h` |
...
| `SaveUserCredentials` and the rest of the user-store nodes | `Blueprint/S3CredentialsLibrary.h` |
...
`MakeCredentialsProviderForSource` is the one place that turns "where to get keys from" into an
...
| The backend gets a burst of key requests | Keys are being set by hand on every operation | Use `FS3CredentialsProvider_Callback` — it coalesces concurrent requests into one |
