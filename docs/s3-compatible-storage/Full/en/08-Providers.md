...
`Expire Orphaned Delete Markers` was added to `FS3LifecycleRule` for exactly this case —
...
- **Lifecycle rules** (`Get`/`Set`/`Delete Bucket Lifecycle`) are also documented as
  ...
  and `ExpireAfterDays` in `FS3LifecycleRule` should work, but `TagFilters` likely won't.
