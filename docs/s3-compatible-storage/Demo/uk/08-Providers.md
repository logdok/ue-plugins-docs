...
| [Backblaze B2](#backblaze-b2) | ✅ | ✅ | ✅ | ✅ | ⚠️ баг B2 | ✅ | ✅ | ⚠️ не перевірено — ключ без прав |
| [Google Cloud Storage](#google-cloud-storage) | ✅ | ✅ | ✅ | ✅ | ❌ 400, не реалізовано | ❌ своя схема | ✅ | ❌ 400, ймовірно бракує IAM-права |
| [MinIO](#minio) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
**`S3 Create Bucket` / `S3 Delete Bucket` не перевірено наживо на B2 в цій сесії** — ключ,
яким тестувався B2, є application key, заскоуплений на конкретний бакет (звична практика
для B2), і на створення нового бакета в акаунті прав не мав: провайдер відповів
`403 AccessDenied`, що плагін і мав розпізнати правильно. Якщо ваш ключ має права на рівні
акаунта — обидві ноди мають працювати так само, як на решті провайдерів; якщо ні, це
налаштування самого ключа B2, не плагіна.
...
**`S3 Create Bucket` відповідає `400 InvalidArgument` через HMAC-ключ.** Відтворено і
плагіном, і незалежно — тим самим ключем через `mc mb` (MinIO Client), без жодного коду
плагіна в шляху, тож це не дефект побудови запиту з нашого боку. Найімовірніша причина:
роль, яку GCS-консоль пропонує при створенні HMAC-ключа (`Storage Object Admin`), діє на
рівні бакета й не включає право `storage.buckets.create`, потрібне на рівні проєкту.
Створюйте бакети для GCS в консолі Google Cloud Storage; `S3 Delete Bucket` на порожньому
бакеті цим не обмежений і в тестах не перевірявся окремо, оскільки без `S3 Create Bucket`
для нього просто не було з чого починати.
