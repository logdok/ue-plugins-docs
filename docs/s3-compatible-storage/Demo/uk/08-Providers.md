*[🇬🇧 English](../en/08-Providers.md) | 🇺🇦 Українська*
...
`Expire Orphaned Delete Markers` додано до `FDemoS3LifecycleRule` саме для цього випадку —
...
- **Правила життєвого циклу** (`Get`/`Set`/`Delete Bucket Lifecycle`) також заявлені як
  ...
  `Prefix` і `ExpireAfterDays` у `FDemoS3LifecycleRule` мають спрацювати, а `TagFilters` —
  імовірно ні.
