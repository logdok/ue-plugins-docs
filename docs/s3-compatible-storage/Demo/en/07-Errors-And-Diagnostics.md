...
The plugin writes to the `LogDemoS3` category.

```
Log LogDemoS3 Verbose       // one line per request: method, address
Log LogDemoS3 VeryVerbose   // plus every signed header
```
...
1. Turn on `Log LogDemoS3 VeryVerbose` and make one request.
