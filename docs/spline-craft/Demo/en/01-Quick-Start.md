...
**From Fab / Epic Games Launcher.** Find SplineCraft Demo in your library, click *Install to
Engine* and pick engine version 5.8. The plugin installs into the engine folder.

**Manual.** Copy the `SplineCraftDemo` folder into `<Project>/Plugins/` (create `Plugins` if it
...
1. **Edit → Plugins**.
2. Search for `SplineCraftDemo`.
3. Tick **Enabled**.
4. Restart the editor when prompted.

## A first structure in 5 steps

1. **Place the actor.** In the *Place Actors* browser find **SplineCraft Demo Actor** and drag it
...
2. **Shape the spline.** Select the actor and move the spline points in the viewport, or open
   **SplineCraft Demo Configuration → Utils → Alignment Spline Points**, pick a **Tool** (for
...
- **Presets.** Instead of configuring by hand you can start from a ready style: assign a
  **SplineCraft Demo Preset** asset to the **Preset** field and press **Apply Preset**. Example:
