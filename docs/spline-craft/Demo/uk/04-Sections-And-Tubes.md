...
Обидві секції використовують той самий тип конфігурації (`FDemoSCElement`); відмінність — лише
призначення та типові меші: секції — це панелі, заповнення, щити; труби — поручні, рейки,
перекладини.

## Елемент (`FDemoSCElement`)
...
## Головна конфігурація меша (`FDemoSCElementMeshConfig`)
...
| **Mode** (`EDemoSplineCraftMode`) | `Line` або `Curve` для цього конкретного меша — застосовується, коли глобальний **Mode** актора = `Concrete`. Див. [Режими Line та Curve](09-Line-vs-Curve-Mode.md). |
| **Ignore … Randomization Settings** | окремо вимкнути рандомізацію масштабу / відступів / зсуву / повороту. |
| **Randomization Settings** (`FDemoSCElementRandomSettings`) | розкид — див. [Рандомізація](08-Randomization.md). |
...
## Вузли на елементі (`FDemoSCElementKnob`)
...
| **Shift** (`FDemoSCShift`) | зсув Horizontal / Vertical (см). |
