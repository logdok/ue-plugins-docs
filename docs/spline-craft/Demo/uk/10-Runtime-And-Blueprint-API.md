...
## Колізія (`FDemoSCCollisionSettings`)
...
`FDemoSplineCraftStaticMeshHitDelegate` (`BlueprintAssignable`), прив'язуються у `BeginPlay`:
...
## Пресети (`UDemoSplineCraftPreset`)
...
Ассет **SplineCraft Demo Build Scenario** (`UDemoSplineCraftBuildScenario`) — упорядкована послідовність
пресетів із таймінгом. Актор із призначеним сценарієм стає **директором**: власної геометрії
не показує, а спавнить по дочірньому SplineCraft Demo-актору на кожен крок — уздовж свого сплайна,
...
`UDemoSplineCraftBuildScenario`: **Description**, **Start Delay** (с), **Loop** (+ **Loop Restart
Delay**, лише пряме програвання), **Steps**.

Крок (`FDemoSCBuildStep`): **Label**, **Preset**, **Delay Before Show** (с), **Offset**
(`FTransform` відносно директора — піднімайте по Z для поверхів), **Appear Mode**,
**Appear Duration** (с), **Rise Distance** (см).

Режими появи (`EDemoSCScenarioAppearMode`): **Instant**, **Rise From Below** (виїжджає знизу на
