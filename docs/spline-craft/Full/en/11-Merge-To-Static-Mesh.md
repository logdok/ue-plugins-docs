...
4. If needed, turn on **Replace Spline Actor** *before* the merge — then the SplineCraft actor
...
- **Irreversible.** The baked mesh is not linked to the actor. To change the fence you have
  to bring the SplineCraft actor back (if Replace Spline Actor was on — from history or from
