...
- The plugin was renamed from **"SplineCraft PRO"** to **"SplineCraft"**.
- **Backward compatibility is preserved.** Unchanged: the module name (`SplineCraft`), the
  API macro (`SPLINECRAFT_API`), the content mount point (`/SplineCraft/...`), the class paths
  (`/Script/SplineCraft.*`), and the layout of properties and structs (`FSC*`). Existing
  levels, Blueprint subclasses of `ASplineCraftActor` and asset references keep working as-is.
- Only the plugin folder name (`SplineCraftPRO` → `SplineCraft`) and the `FriendlyName`
  changed.
...
- **Build Scenario — building step by step.** A new **SplineCraft Build Scenario** asset: an
  ...
  SplineCraft actors one by one along its own spline, each with its own offset — so one
...
- **Style presets.** A new **SplineCraft Preset** asset type stores a whole structure setup:
...
- **`FSCPolygon::Material` had no initializer**, which caused a validation error on startup
