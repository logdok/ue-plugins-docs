...
## Main Static Mesh Configuration (`FSCPointMeshConfig`)
...
| **Displayed Object** (`EShowKnobMode`) | `Mesh` — show a mesh; `Actor` — spawn an actor (visible in game only). See [Additional Actors](12-Additional-Actors.md). |
| **Actor** (`FSCAdditionalActor`) | the actor class when `Actor` mode is chosen. |
