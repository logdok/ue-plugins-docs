...
## The plugin used to be called "SplineCraft PRO" — is it the same product?

Yes. The plugin was renamed to **SplineCraft**. Compatibility is preserved: the module name
(`SplineCraft`), the content mount point (`/SplineCraft/...`), the class paths
(`/Script/SplineCraft.*`) and the property layout are unchanged. Existing levels, Blueprint
subclasses of `ASplineCraftActor` and asset references keep working. See
...
structure's parameters are no longer editable — keep the source SplineCraft actor if you plan
...
`SplineCraft Preset` asset, and **Apply Preset** on another actor to load the style. A preset
